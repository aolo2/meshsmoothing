...
static const char *fs_source_ui = "#version 330 core\n"
"out vec4 frag_color;\n"
"void main()\n"
"{\n"
...
static s32
ms_opengl_init_shader_program_ui()
{
    char info_log[512];
    
    u32 vertex_shader = glCreateShader(GL_VERTEX_SHADER);
    glShaderSource(vertex_shader, 1, &vs_source_ui, NULL);
    glCompileShader(vertex_shader);
    _check_shader_error(vertex_shader, info_log, "Vertex shader", GL_COMPILE_STATUS);
    
    u32 fragment_shader = glCreateShader(GL_FRAGMENT_SHADER);
    glShaderSource(fragment_shader, 1, &fs_source_ui, NULL);
    glCompileShader(fragment_shader);
    _check_shader_error(fragment_shader, info_log, "Fragment shader", GL_COMPILE_STATUS);
    ...
    //printf("[INFO] Compiled shaders (UI)\n");
...
static void
ms_opengl_flatquad(f32 x1, f32 y1, f32 x2, f32 y2)
{
    u32 VAO, VBO;
    f32 vertices[] = { 
        x1, y1, x1, y2,
    ...
    glBufferData(GL_ARRAY_BUFFER, 16 * sizeof(f32), vertices, GL_STATIC_DRAW);
    glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 2 * sizeof(f32), NULL);
    glEnableVertexAttribArray(0);
    
    /* TODO: compile and save shaders */
    s32 shader_program_ui = ms_opengl_init_shader_program_ui();
...
ms_opengl_init_buffers(struct ms_mesh mesh, struct ms_v3 **triangulated_points)
{
    ...
    *triangulated_points = triangle_verts;
...
static void
ms_opengl_update_buffers(struct ms_gl_bufs bufs, struct ms_v3 **triangulated_points, struct ms_mesh mesh)
{
